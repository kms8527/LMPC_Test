...
typedef struct Point_ref {
    double x;
    double y;
    double theta; // theta represents progress along centerline_points interval = 0.5 ( 0, 0.5, 1.0, 1.5, ...)
...
class Track {
  public:
    ...
    tk::spline X_spline;
    tk::spline Y_spline;
    double length; // length of global path
    double space;
    ...
    Track(string file_name, nav_msgs::OccupancyGrid &map, bool sparse = false)
        ...
        std::cout << "Print content of CSV File:\n";
        ...
        while (next < waypoints.size()) {
            float dist =
                sqrt(pow(waypoints.at(next).x - waypoints.at(curr).x, 2) +
                     pow(waypoints.at(next).y - waypoints.at(curr).y, 2));
            float dist_to_start =
                sqrt(pow(waypoints.at(next).x - waypoints.at(0).x, 2) +
                     pow(waypoints.at(next).y - waypoints.at(0).y, 2));
            if (dist > space) {
                theta += dist;
                Point_ref p;
                p.x = waypoints.at(next).x;
                p.y = waypoints.at(next).y;
                p.theta = theta;
        ...
        double last_space =
            sqrt(pow(centerline_points.back().x - waypoints.at(0).x, 2) +
        ...
        p_second_last.x = 0.5 * (centerline_points.back().x + p_last.x); // (first waypoint + last waypoint) / 2
        p_second_last.y = 0.5 * (centerline_points.back().y + p_last.y); // (first waypoint + last waypoint) / 2
        p_second_last.theta = length - 0.5 * last_space;

        if (last_space > space) {
            centerline_points.push_back(p_second_last); // add last intermediate point
        }
        ...
        X_spline.set_points(thetas, X); // theta = 
        Y_spline.set_points(thetas, Y);

        /** if sparse, reinitialize centerline points such that they are densely
         * and equally spaced **/
        if (sparse) {
            double s = 0;
            centerline_points.clear();
            while (s < length) {
                ...
                p.theta = s;
                s += space;
                centerline_points.push_back(p); // regenerate centerline points using spline
            }
        }
        initialize_width();
    }
    ...
    void initialize_width() {
        ...
        for (int i = 0; i < centerline_points.size(); i++) {
            ...
            while (true) {
                double x =
                    (p + t * map.info.resolution *
                             Vector2d(dy_dtheta, -dx_dtheta).normalized())(0);
                double y =
                    (p + t * map.info.resolution *
                             Vector2d(dy_dtheta, -dx_dtheta).normalized())(1);
    ...
    double findTheta(double x, double y, double theta_guess,
                     bool global_search = false) {
        /* return: projected theta along centerline_points, theta is between [0,
         * length]
         * */
        ...
        //                double dist2 = pow(x-centerline_points.at(i).x, 2) +
        //                pow(y-centerline_points.at(i).y, 2); if( dist2 <
        //                min_dist2){
        //                    min_dist2 = dist2;
        //                    min_ind = i;
        //                }
        ...
        //            if (min_ind_next>centerline_points.size()-1){ min_ind_next
        //            -= centerline_points.size();} if (min_ind_prev<0){
        //            min_ind_prev += centerline_points.size();}
        //
        //            //closest line segment: either [min_ind ,min_ind+1] or
        //            [min_ind,min_ind-1] if
        //            (pow(x-centerline_points.at(min_ind_next).x, 2) +
        //            pow(y-centerline_points.at(min_ind_next).y, 2) <
        //                    pow(x-centerline_points.at(min_ind_prev).x, 2) +
        //                    pow(y-centerline_points.at(min_ind_prev).y, 2)){
        ...
        //
        //            p(0) = x;  p(1) = y;
        //            p0(0) = centerline_points.at(min_ind).x;  p0(1) =
        //            centerline_points.at(min_ind).y; p1(0) =
        //            centerline_points.at(second_min_ind).x;  p1(1) =
        //            centerline_points.at(second_min_ind).y;
        //
        //            double projection = abs((p - p0).dot(p1 - p0)/(p1 -
        //            p0).norm()); double theta;
        //
        //            if (min_ind > second_min_ind){
        //                theta = centerline_points.at(min_ind).theta -
        //                projection;
        //            }
        //            else {
        //                if (min_ind == 0 && second_min_ind ==
        ...
        for (int i = 0; i < centerline_points.size(); i++) {
            double dist2 = pow(x - centerline_points.at(i).x, 2) +
                           pow(y - centerline_points.at(i).y, 2);
            if (dist2 < min_dist2) {
                min_dist2 = dist2;
                min_ind = i;
            }
    ...
    double getcenterline_pointsCurvature(double theta) {
        return (X_spline.eval_d(theta) * Y_spline.eval_dd(theta) -
                Y_spline.eval_d(theta) * X_spline.eval_dd(theta)) /
               pow((pow(X_spline.eval_d(theta), 2) +
                    pow(Y_spline.eval_d(theta), 2)),
                   1.5);
    }
